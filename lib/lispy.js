        ...
        list: (values) => { 
          return State.state(env => {
            return array.match(values, {
              ...
              cons: (head, tail) => {
                return Maybe.flatMap(State.eval(Cont.eval(Lispy.Semantics.evaluate(head)))(env))(first => {
                //return Maybe.flatMap(State.eval(Lispy.Semantics.evaluate(head))(env))(first => {
                  const items = array.foldl(tail)([first])(item => {
                    return (accumulator) => {
                      return Maybe.flatMap(State.eval(Cont.eval(Lispy.Semantics.evaluate(item)))(env))(value => {
                      // return Maybe.flatMap(State.eval(Lispy.Semantics.evaluate(item))(env))(value => {

...
const Lispy = {
  Context: {
    // lookup: STRING -> State Env Maybe[VALUE]
    lookup: (key) => {
      return ST.flatMap(ST.get())(env => {
        return ST.unit(Env.lookup(key)(env));
      });
    },
    // extend: (STRING, VALUE) -> State Env Env
    extend: (key, value) => {
      return ST.flatMap(ST.get())(env => {
        return ST.put(Env.extend(key, value)(env));
      });
    }
  },
  REPL: {

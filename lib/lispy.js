        ...
        list: (values) => { 
          return State.state(env => {
            return array.match(values, {
              ...
              cons: (head, tail) => {
                return Maybe.flatMap(Cont.eval(State.eval(Lispy.Semantics.evaluate(head))(env)))(first => {
                  const items = array.foldl(tail)([first])(item => {
                    return (accumulator) => {
                      return Maybe.flatMap(Cont.eval(State.eval(Lispy.Semantics.evaluate(item)))(env))(value => {

...
/* ifの式 */
Exp.if =  (predicate, trueExp, falseExp) => {
  expect(variable).to.be.a('function')
  return (pattern) => {
...
const Lispy = {
      ...
      if: (predicate, trueExp, falseExp) => {
        ...
        return State.state(env => {
          return Maybe.flatMap(State.eval(Cont.eval(evaluator(predicate)))(env))(bool => {
            return Exp.match(bool,{
              bool: (value) => {
                if(value) {
                  const maybeValue = State.eval(Cont.eval(evaluator(trueExp)))(env);
                  return pair.cons(maybeValue, env); 
                } else {
                  const maybeValue = State.eval(Cont.eval(evaluator(falseExp)))(env);
                  return pair.cons(maybeValue, env); 
                }
              }
            });
  ...
  Syntax: {
    ...
    special: () => {
      const open = Parser.char("{"), close = Parser.char("}"); 
      const identifier = (_) => {
        return Parser.flatMap(Parser.token(Parser.ident()))(name => {
          return Parser.unit(name);
        });
      };
      return Parser.flatMap(open)(_ => { 
        return Parser.flatMap(identifier())(name => {
          switch (name) {
            case "set": // {set variable expression}
              return Parser.flatMap(identifier())(name => {
                return Parser.flatMap(Lispy.Syntax.expression())(body => {
                  return Parser.flatMap(close)(_ => {

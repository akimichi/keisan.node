...
const Lispy = {
  ...
  Semantics: {
    ...
    binary: (operator) => (expL, expR) => (env) => {
      return Maybe.flatMap(State.eval(Cont.eval(Lispy.Semantics.evaluate(expL)(Lispy.Semantics.definition)))(env))(valueL => {
        return Maybe.flatMap(State.eval(Cont.eval(Lispy.Semantics.evaluate(expR)(Lispy.Semantics.definition)))(env))(valueR => {
    ...
    definition: {
      ...
      list: (values) => { 
        return State.state(env => {
          return array.match(values, {
            ...
            cons: (head, tail) => {
              return Maybe.flatMap(State.eval(Cont.eval(Lispy.Semantics.evaluate(head)(Lispy.Semantics.definition)))(env))(first => {
                const items = array.foldl(tail)([first])(item => {
                  return (accumulator) => {
                    return Maybe.flatMap(State.eval(Cont.eval(Lispy.Semantics.evaluate(item)(Lispy.Semantics.definition)))(env))(value => {
          ...
          return Exp.match(identifier,{
            variable: (name) => {
              const closure = (actualArg => {
                ...
                return State.eval(Cont.eval(Lispy.Semantics.evaluate(body)(Lispy.Semantics.definition)))(localEnv);
              });
      ...
      app: (operator, operand) => {
        return State.state(env => {
          return Maybe.flatMap(State.eval(Cont.eval(Lispy.Semantics.evaluate(operator)(Lispy.Semantics.definition)))(env))(closure => {
            return Maybe.flatMap(State.eval(Cont.eval(Lispy.Semantics.evaluate(operand)(Lispy.Semantics.definition)))(env))(actualArg => {

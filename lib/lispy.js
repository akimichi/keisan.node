...
Exp.set =  (variable, body) => {
  ...
  const matcher = (pattern) => pattern.set(variable, body);
  return tap(matcher, (expression) => {
    expression.type = "set";
    expression.content = {
      variable: variable,
      body: body
...
Exp.condition =  (predicate, ifExp, elseExp) => {
  ...
  const matcher =  (pattern) => pattern.condition(predicate, ifExp, elseExp);
  return tap(matcher, (expression) => {
        ...
        return State.state(env => {
          return Maybe.flatMap(State.eval(Cont.eval(evaluator(predicate)))(env))(bool => {
            if(bool) {
              ...
              // const newState = State.eval(Cont.eval(evaluator(falseExp)))(env),
              //   maybeValue = pair.left(newState),
              //   newEnv = pair.right(newState);
              // return Maybe.flatMap(maybeValue)(value => {
              //   console.log(`value: ${value}`)
              //   return pair.cons(newState, env); 
              // })
            }
          });
        });
        ...
        return State.state(env => {
          return Exp.match(identifier,{
            variable: (name) => {
              ...
              //   return Maybe.just(State.eval(Cont.eval(evaluator(body)))(localEnv));
              // });
